...
```python
al = A[i - 1] if i > 0 else float('-inf')  # al = A[1] = 3
ar = A[i] if i < len(A) else float('inf')  # ar = A[2] = 8

bl = B[j - 1] if j > 0 else float('-inf')  # bl = B[2] = 18
br = B[j] if j < len(B) else float('inf')  # br = B[3] = 19
```
...
- **Condition**: Is `al <= br` and `bl <= ar`?
  - `3 <= 19` (True)
  - `18 <= 8` (False)
- Since `bl > ar` (`18 > 8`), we need to move `l` to `i + 1`:
...
```python
al = A[i - 1] if i > 0 else float('-inf')  # al = A[3] = 9
ar = A[i] if i < len(A) else float('inf')  # ar = A[4] = 15

bl = B[j - 1] if j > 0 else float('-inf')  # bl = B[0] = 7
br = B[j] if j < len(B) else float('inf')  # br = B[1] = 11
```
...
- **Adjusting `l` and `r`**:
  ...
  - If `al > br`, it means there are too many large elements on the left side of `A`. We need to move `r` left to reduce `i`.
  - If `bl > ar`, there are too many small elements on the left side of `B`. We need to move `l` right to increase `i`.
...
- **When `i = 0` or `i = len(A)`**:
  - If `i = 0`, it means there are no elements on the left side of `A`; we set `al = float('-inf')`.
  - If `i = len(A)`, there are no elements on the right side of `A`; we set `ar = float('inf')`.

- **This ensures that comparisons like `al <= br` and `bl <= ar` work correctly, even when partitions are at array boundaries.**
...
2. **First Iteration**:
   ...
   - `al <= br`? `1 <= 4` (True)
   - `bl <= ar`? `3 <= 2` (False)
   - Since `bl > ar`, move `l` to `i + 1 = 2`

3. **Second Iteration**:

   - `i = (2 + 2) // 2 = 2`
   - `j = 2 - 2 = 0`
   - `al = A[1] = 2`, `ar = float('inf')` (since `i = len(A)`)
   - `bl = float('-inf')` (since `j = 0`), `br = B[0] = 3`
   - `al <= br`? `2 <= 3` (True)
   - `bl <= ar`? `-inf <= inf` (True)
   - Correct partition found.
