...
1. All children inherit the bounds from parents.
2. Update the inherited bounds based on which side you are going.  
    If you are going *left*, update **right bound with parent** because you can't be greater than parent.   
    If you are going *right*, update **left bound with parent** because you can't be smaller than parent.

---


Trick to remember/memorise the bounds.

**“Left is Less, Right is Greater.”**

- **Left → Less than parent → update the *high* bound**  
- **Right → Greater than parent → update the *low* bound**

#### 📝 Mental Shortcut
Whenever you’re about to recurse, ask yourself:
- “Am I going left? Then I must shrink the ceiling.”  
- “Am I going right? Then I must raise the floor.”

#### 🔁 Practice Drill
Here’s a quick way to reinforce:
1. Write down a small BST (say root = 10, left = 5, right = 15).
2. Walk through it manually, updating bounds at each step.
3. Say out loud:  
   - “Going left → upper bound = parent”  
   - “Going right → lower bound = parent”
...
If the interviewer asks you to sort the intervals without using in-built sorting
methods. Then use the [Merge Sort Template](../foundation/sort.md#generic-merge-sort-template-with-lambda-key).
