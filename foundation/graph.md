...
Dijkstra's algorithm is a greedy algorithm that finds the shortest path from a source node to all other nodes in a graph with non-negative edge weights. It uses a priority queue to always process the node with the smallest tentative distance first.

**Key Steps:**

1. **Initialization**: Set distance to source as 0, others as infinity. Create adjacency list from edges.

2. **Priority Queue**: Use a min-heap to store (distance, node) pairs, starting with (0, src).

3. **Processing**: While heap is not empty:
   - Extract the node with smallest distance.
   - Skip if this distance is outdated (greater than current dist[node]).
   - For each neighbor, if a shorter path is found via this node, update distance and push new (distance, neighbor) to heap.

4. **Result**: Distances array, with unreachable nodes as -1.

**Why the `if d > dist[u]` check?** This handles the fact that Python's heapq doesn't support decreasing keys. When we update a distance, we push a new entry, and the old larger distance might still be in the heap. We skip processing outdated entries.

#### Time Complexity

- **O((V + E) log V)**: Each edge is processed once, and heap operations (push/pop) are log V.

#### Space Complexity

- **O(V + E)**: Adjacency list and distance array. Heap can hold up to O(V) elements.

#### When to Use Dijkstra vs Bellman-Ford

- **Dijkstra**: For graphs with non-negative weights. Faster and simpler.
- **Bellman-Ford**: Handles negative weights, detects negative cycles. Slower, O(V * E).

#### Assumptions

- Edge weights must be non-negative. Negative weights can cause incorrect results.
- Graph can be directed or undirected (commented in code for undirected).
